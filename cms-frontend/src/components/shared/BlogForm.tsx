...
import { useBlogForm, BlogFormValues } from "../..//lib/hooks/useBlogForm";
import { Input } from "../ui/Input";
import { Button } from "../ui/Button";

function Inner({ onSubmit }: { onSubmit: (v: BlogFormValues) => Promise<void> }) {
  const { register, handleSubmit, watch, setValue } = useFormContext<BlogFormValues>();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <Input label="Slug" {...register("slug")} placeholder="unique-blog-slug" />
          ...
          {(() => {
            ...
            return (
              <div key={lang} className="mb-6 p-4 border border-gray-300 rounded-lg bg-gray-50">
                ...
                <Input label="Title" {...register(`translations.${idx}.title`)} placeholder="Blog title" className="text-gray-900" />
                <div className="mt-2">
...
export function BlogForm({ initialValues, onSaved }: { initialValues?: Partial<BlogFormValues>; onSaved?: () => void }) {
  const { methods, submit } = useBlogForm(initialValues || {}, () => { if (onSaved) onSaved(); });
  const handleSubmitWrapped = async (v: BlogFormValues) => {
